...
class Yandex:
    ...
    def _create_folder(self, folder_name):
        ...
        if requests.get(url, headers=self.headers, params=params).status_code != 200:
            ...
            print(f'\nПапка {folder_name} создана в корневом каталоге Яндекс.Диск\n')
        else:
    ...
    def create_copy(self, dict_files):
        ...
        print(f'\nЗапрос завершен. Количество скопированных новых файлов: {copy_counter}'
              f'\nВсего файлов в исходном альбоме ВКонтакте: {len(dict_files)}')


if __name__ == '__main__':

    tokenVK = 'vk_token.txt'
    tokenYandex = 'yandex_disk_token.txt'
    ...
    my_yandex = Yandex('VK photo', get_token_id(tokenYandex), 5)
